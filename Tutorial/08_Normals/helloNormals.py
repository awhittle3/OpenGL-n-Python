...
class Virus(GameObject):
    ...
    def update(self):
        self.rotate(2.0, np.array([1.0, 1.0, 0.0]))
...
def main():
    ...
    glutCreateWindow(b'Hello Normals')
    ...
    virus = Virus()
    virus.translate(np.array([-1.5,0.0,0.0]))
    normVirus = Virus()
    normVirus.translate(np.array([1.5,0.0,0.0]))

    triangleShader = ShaderProgram([virus], 'simpleMVP.vert', 'simpleMVP.frag')
    normalShader = ShaderProgram([normVirus], 'normalViz.vert', 'normalViz.frag')

    programs = [triangleShader, normalShader]

    projMatrix = perspective(fieldOfView, float(screenWidth)/float(screenHeight), nearClipPlane, farClipPlane)
    viewMatrix = lookAt(np.array([0.0,0.0,5.0]), np.array([0.0,0.0,0.0]), np.array([0.0,1.0,0.0]))
